...
int builtin_cmd(char** argv)
{
	...
	if (!strcmp(argv[0], "jobs"))    //如果为jobs，用listjobs输出作业列表的所有作业信息
	{
		listjobs(jobs);
		return 1;
	}
	if (!strcmp(argv[0], "bg") || !strcmp(argv[0], "fg"))  //如果为bg或fg，调用do_bgfg执行bg和fg两天内置指令
	{
		do_bgfg(argv);
		return 1;
	}
...
void sigchld_handler(int sig)
{
	...
	struct job_t* jobnew;   //新建job_t的结构体指针job_new
	sigset_t mask, prev_mask;
	int olderrno = errno, status;
	pid_t pid;
	sigfillset(&mask);   //将每个信号都添加到mask中
	while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)   //尽可能回收子进程
	{
		sigprocmask(SIG_BLOCK, &mask, &prev_mask);  //需要deletejob，阻塞所有信号
		jobnew = getjobpid(jobs, pid);  //通过pid找到作业job
